# Agent Please Service Specification
...
Agent Please is a long-running automation service that continuously reads work from an issue tracker
...
- Agent Please is a scheduler/runner and tracker reader.
...
Agent Please is easiest to port when kept in these layers:
...
- `root` (path string or `$VAR`)
  - Default: `<system-temp>/agent-please_workspaces`
...
- `workspace.root`: path, default `<system-temp>/agent-please_workspaces`
...
```json
{"id":1,"method":"initialize","params":{"clientInfo":{"name":"agent-please","version":"1.0"},"capabilities":{}}}
...
- Purpose: execute a raw REST API call against Asana using Agent Please's configured tracker auth for
  the current session.
...
- Reuse the configured Asana endpoint and auth from the active Agent Please workflow/runtime config; do
...
- Purpose: execute a raw GraphQL query or mutation against GitHub using Agent Please's configured
...
- Reuse the configured GitHub endpoint and auth from the active Agent Please workflow/runtime config;
...
Agent Please does not require first-class tracker write APIs in the orchestrator.
...
- `GET /api/v1/<issue_identifier>`
  ...
  - Suggested response shape:
    ...
    ```json
    {
      ...
      "status": "running",
      "workspace": {
        "path": "/tmp/agent-please_workspaces/MT-649"
      },
      "attempts": {
        "restart_count": 1,
      ...
      "logs": {
        "agent_session_logs": [
          {
            "label": "latest",
            "path": "/var/log/agent-please/agent/MT-649/latest.log",
            "url": null
          }
        ]
      },
...
- Optional `asana_api` client-side tool extension exposes Asana REST API access through the
  app-server session using configured Agent Please auth (only when `tracker.kind=asana`).
- Optional `github_graphql` client-side tool extension exposes GitHub GraphQL access through the
  app-server session using configured Agent Please auth (only when `tracker.kind=github_projects`).
