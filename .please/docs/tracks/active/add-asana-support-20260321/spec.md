...
- [ ] FR-3: **Asana Write Support** — Implement `updateItemStatus()` in the Asana tracker adapter to move tasks between sections (POST `/tasks/{gid}/addProject` with `section` parameter). Replace the current `tracker_write_not_supported` stub.
