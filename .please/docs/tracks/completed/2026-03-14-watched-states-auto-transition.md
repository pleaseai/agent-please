...
- [ ] T001 Add watched_states and auto_transitions to config types and parsing (file: apps/agent-please/src/types.ts, apps/agent-please/src/config.ts)
- [ ] T002 Add has_unresolved_threads and unresolved_thread_authors to Issue type (file: apps/agent-please/src/types.ts)
- [ ] T003 Add reviewThreads to GitHub GraphQL queries and normalize into Issue (file: apps/agent-please/src/tracker/github.ts) (depends on T002)
- [ ] T004 Add updateItemStatus to TrackerAdapter interface (file: apps/agent-please/src/tracker/types.ts)
- [ ] T005 Implement updateItemStatus for GitHub adapter with field/option ID caching (file: apps/agent-please/src/tracker/github.ts) (depends on T004)
- [ ] T006 Implement stub updateItemStatus for Asana adapter (file: apps/agent-please/src/tracker/asana.ts) (depends on T004)
- [ ] T007 Add getWatchedStates and getAutoTransitions config helpers (file: apps/agent-please/src/config.ts) (depends on T001)
- [ ] T008 Implement processWatchedStates in orchestrator tick loop (file: apps/agent-please/src/orchestrator.ts) (depends on T003, T005, T007)
- [x] T009 Update generateWorkflow template with watched_states and auto_transitions (file: apps/agent-please/src/init.ts) (depends on T001)
- [ ] T010 Update WORKFLOW.md and README.md with new config fields (file: WORKFLOW.md, README.md) (depends on T009)
- [ ] T011 [P] Add config tests for watched_states and auto_transitions parsing (file: apps/agent-please/src/config.test.ts) (depends on T001, T007)
- [ ] T012 [P] Add tracker tests for reviewThreads normalization and updateItemStatus (file: apps/agent-please/src/tracker/tracker.test.ts) (depends on T003, T005)
- [ ] T013 Add orchestrator tests for processWatchedStates (file: apps/agent-please/src/orchestrator.test.ts) (depends on T008)
...
### Modify
- `apps/agent-please/src/types.ts` — Add `watched_statuses`, `auto_transitions` to TrackerConfig; add `has_unresolved_threads` to Issue
- `apps/agent-please/src/config.ts` — Parse new config fields, add `getWatchedStates()`, `getAutoTransitions()` helpers, add defaults
- `apps/agent-please/src/tracker/types.ts` — Add `updateItemStatus()` to TrackerAdapter interface
- `apps/agent-please/src/tracker/github.ts` — Add `reviewThreads` to GraphQL fragments, implement `updateItemStatus()` with field/option ID caching
- `apps/agent-please/src/tracker/asana.ts` — Add stub `updateItemStatus()` returning error
- `apps/agent-please/src/orchestrator.ts` — Add `processWatchedStates()` method in tick loop
- `apps/agent-please/src/init.ts` — Update `generateWorkflow()` template, `STATUS_OPTIONS`
- `WORKFLOW.md` — Add `watched_states` and `auto_transitions` config
- `README.md` — Document new config fields and template variables

### Reuse
- `apps/agent-please/src/config.ts:normalizeState()` — state comparison
- `apps/agent-please/src/config.ts:csvValue()` — parse YAML arrays
- `apps/agent-please/src/tracker/github.ts:runGraphql()` — execute GraphQL
- `apps/agent-please/src/tracker/github.ts:createAuthenticatedGraphql()` — auth setup
