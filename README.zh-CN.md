...
Agent Please 将问题追踪器中的任务转化为隔离的自主实现运行 —— 管理工作而非监督编码代理。

> **警告**：Agent Please 是一个工程预览版，仅适用于受信任的环境。
...
Agent Please 是一个长期运行的 TypeScript 服务，它：
...
- **Slack 通知** —— 在 Slack 中 @提及机器人获取编排器状态。使用
  [Chat SDK](https://chat-sdk.dev/) Slack 适配器。
...
# 使用当前目录的 WORKFLOW.md 运行 Agent Please
bunx agent-please
...
`WORKFLOW.md` 是 Agent Please 运行时行为的唯一事实来源。它将 YAML front matter
...
workspace:
  root: ~/agent-please_workspaces        # 可选: 默认 <tmpdir>/agent-please_workspaces
...
claude:
  ...
  settings:
    attribution:
      commit: "🙏 Generated with [Agent Please](https://github.com/pleaseai/agent-please)"  # 可选: 附加到 git 提交消息。默认为 Agent Please 链接。
      pr: "🙏 Generated with [Agent Please](https://github.com/pleaseai/agent-please)"      # 可选: 附加到 PR 描述。默认为 Agent Please 链接。

server:
  port: 3000                          # 可选: 在此端口启用 HTTP 仪表板
  # Webhook 端点:
  #   GitHub: POST /api/webhooks/github (需要 server.webhook.secret)
  #   Slack:  POST /api/webhooks/slack  (需要 SLACK_BOT_TOKEN + SLACK_SIGNING_SECRET 环境变量)
---
...
Agent Please 在启动时验证 GitHub App 配置：
...
Agent Please 通过 [Chat SDK](https://chat-sdk.dev/) Slack 适配器支持 Slack 作为通知渠道。
配置后，您可以在 Slack 频道中 @提及机器人来获取实时编排器状态（运行中的问题、重试队列、
令牌使用量）。
...
SLACK_SIGNING_SECRET=...           # 用于 Webhook 验证的签名密钥
```

当两个环境变量都设置后，Slack 适配器会与已配置的追踪器（GitHub Projects v2 或 Asana）
一起自动启用。

### Webhook URL

将 Slack 应用的 Event Subscriptions 和 Interactivity 请求 URL 设置为：
...
1. 前往 [api.slack.com/apps](https://api.slack.com/apps)，点击 **Create New App** > **From an app manifest**。
2. 选择工作区并粘贴以下清单：

```yaml
display_information:
  name: Agent Please
  description: 编排器状态机器人
features:
  bot_user:
...
3. 将 `https://your-domain.com/api/webhooks/slack` 替换为您部署的 Webhook URL。
4. 点击 **Create**，然后在 **Basic Information** > **App Credentials** 中复制 **Signing Secret** 作为 `SLACK_SIGNING_SECRET`。
5. 在 **OAuth & Permissions** 中点击 **Install to Workspace**，复制 **Bot User OAuth Token**（`xoxb-...`）作为 `SLACK_BOT_TOKEN`。
6. 将机器人邀请到频道并 @提及它以查看编排器状态。

## 信任与安全

Agent Please 自主运行 Claude Code。部署前请了解相关的信任影响。
...
- Agent Please 是基于 OpenAI 的 [Symphony 规范](vendor/symphony/SPEC.md)（Apache 2.0）的 TypeScript 实现。
